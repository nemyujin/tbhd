...
struct CreateNewPost: View {
    ...
    var body: some View {
        ...
        .onChange(of: photoItem) { newValue in
            print("onChange called with photoItem: \(newValue)")
            if let newValue{
                Task{
        ...
        Task {
            do {
                ...
                if let postImageData {
                    ...
                    let post = Post(text: postText, imageURL: downloadURL, imageReferenceID: imageReferenceID, userName: "홍유진", userUID: userUID)
                    print("Post created with userName: ")
                    try await createDocumentAtFirebase(post)
                } else {
                    let post = Post(text: postText, userName: userName, userUID: userUID)
                    try await createDocumentAtFirebase(post)
                }
            } catch {
